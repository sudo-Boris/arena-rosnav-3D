...
class ScanMapper():
    ...
    def callback_flip_laserscan(self, data: LaserScan):
        np_ranges = np.array(data.ranges, dtype=np.float32)
        np_intensities = np.array(data.intensities, dtype=np.float32)
        flipped_r = np.flip(np_ranges)
        flipped_i = np.flip(np_intensities)
        data.ranges = np.roll(flipped_r, 1)
        data.intensities = np.roll(flipped_i, 1)
    ...
    def flip_scan_data(self, data: LaserScan):
        np_ranges = np.array(data.ranges, dtype=np.float32)
        np_intensities = np.array(data.intensities, dtype=np.float32)
        flipped_r = np.flip(np_ranges)
        flipped_i = np.flip(np_intensities)
